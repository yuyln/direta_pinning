module variaveis
    ...
    integer, parameter:: ni = 50000, ns = 2, np = 384, nc = 1000, &
&                        corte = 50, relax = 100000
    integer:: nxs, nys, cxs, cys
    integer, dimension(:, :, :), allocatable:: caixa
    integer, dimension(:, :), allocatable:: ncaixa
    integer, dimension(:, :, :), allocatable:: caixa_skyr
    integer, dimension(:, :), allocatable:: ncaixa_skyr
    doubleprecision, parameter:: fator = 1.d-6
    doubleprecision, parameter:: tf = 5000.d0, hs = tf / ni, &
    &                            pi = dacos(-1.d0), lx = 36.d0, &
    &                            ly = lx * dsqrt(3.d0) / 2.d0, xhi = 1.d0, &
    &                            temp = 0.d0, fcf = 2.d0, &
    &                            ac = 0.d0, &
    &                            u = 1.d0, rp = 0.65d0, &
    &                            hc = fcf / nc
    doubleprecision, parameter:: nac = 100.d0, omega = dble(nac) / dble(ni)
    doubleprecision, parameter:: betadamp = 0.5d0, &
...
&                                beta = betadamp * damping
    ...
    doubleprecision:: ssx, ssy, vpx, vpy, vx, vy, inicio, fim, fc, vmx, vmy
    doubleprecision:: k1x, k1y, k2x, k2y, fx, fy, na, p1, p2, x_temp, y_temp
    doubleprecision:: fcx, fcy, aux, hx, hy, hxs, hys
...
module gera_function
    ...
    doubleprecision, dimension(0: n):: y_bessel, y_exp
    doubleprecision, parameter:: ae = 0.d0, ab = 0.5d0
    doubleprecision::bb, be, hb, he
    ...
    doubleprecision function tab_exp(x)
        implicit none
        integer:: index_
        doubleprecision:: x
        if (x > be) then
...
subroutine cortes
    ...
    do i = 0, n
        aux = dble(i) * 15.d0 / dble(n)
        if (dexp(-aux ** 2.d0 / (rp ** 2.d0)) * u <= fator) then
            be = aux
            he = (be - ae) / n
            exit
        endif
...
subroutine ler_pin(np, x_pin, y_pin, lx, ly, caixa, nx, ny, ncaixa, hx, hy)
    implicit none
    integer:: np, ip, nx, ny, i, j, cont
    doubleprecision, dimension(1: np):: x_pin, y_pin
    integer, dimension(1: nx, 1: ny, 1: np):: caixa
    integer, dimension(1: nx, 1:ny):: ncaixa
    doubleprecision:: lx, ly, xr, yr, hx, hy
    logical:: caixax, caixay
    open(1, file='pin.dat', status='old')
    caixa(:, :, :) = -1
    ncaixa(:, :) = 0
    ...
    do i = 1, nx
        do j = 1, ny
            cont = 0
            do ip = 1, np
                caixax = (x_pin(ip) < (i * hx)).and.(x_pin(ip) >= (i - 1) * hx)
                caixay = (y_pin(ip) < (j * hy)).and.(y_pin(ip) >= (j - 1) * hy)
                if (caixax.and.caixay) then
                    cont = cont + 1
                    caixa(i, j, cont) = ip
                    ncaixa(i, j) = ncaixa(i, j) + 1
                endif
            enddo
        enddo
    enddo
...
subroutine skyr_pinning(x0, y0, vpx, vpy, tab_exp, x_pin, y_pin, u, rp)
    implicit none
    doubleprecision:: x0, y0
    doubleprecision:: x_pin, y_pin
    doubleprecision:: vpx, u, vpy
    doubleprecision:: dx, dy, d, d2
    doubleprecision:: rp
    doubleprecision, external:: tab_exp
    call distancia(x_pin, &
    &              y_pin, &
    &              x0, &
    &              y0, &
    &              dx, dy, d, d2)
    vpx = vpx - 2.d0 * u / (rp ** 2.d0) * dx * tab_exp(d2)
    vpy = vpy - 2.d0 * u / (rp ** 2.d0) * dy * tab_exp(d2)
end subroutine
...
subroutine forcas(desx, desy)
    ...
    cx = nint((x0(i_s) + desx) / (lx / nx)) + 1
    cy = nint((y0(i_s) + desy) / (ly / ny)) + 1
    if (cx < 1) cx = 1
    if (cy < 1) cy = 1
    if (cx > nx) cx = nx
    if (cy > ny) cy = ny
    ...
    do i__ = -1, 1
        ...
        do j = -1, 1
            ...
            if (baixo) enty = ny
            if (.not.cima.and..not.baixo.and..not.esq.and..not.dir) then
                do i_ = 1, ncaixa(entx, enty)
                    ip = caixa(entx, enty, i_)
                    if ((dabs(x_pin(ip) - (x0(i_s) + desx)) <= be).and.((dabs(y_pin(ip) - (y0(i_s) + desy)) <= be))) then
                        call skyr_pinning(x0(i_s) + desx, (y0(i_s) + desy), vpx, vpy, tab_exp, x_pin(ip), y_pin(ip), u, rp)
                    endif
                enddo
            endif
            ...
            if (cima) then
                ...
                do i_ = 1, ncaixa(entx, enty)
                    ip = caixa(entx, enty, i_)
                    if ((dabs(x_pin(ip) - (x0(i_s) + desx)) <= be).and.((dabs(y_pin(ip) + ly -(y0(i_s) + desy)) <= be))) then
                        call skyr_pinning((x0(i_s) + desx), (y0(i_s) + desy), vpx, vpy, tab_exp, x_pin(ip), y_pin(ip) + ly, u, rp)
                    endif
                enddo
            endif
            ...
            if (esq) then
                ...
                do i_ = 1, ncaixa(entx, enty)
                    ip = caixa(entx, enty, i_)
                    if ((dabs(x_pin(ip) - lx - (x0(i_s) + desx)) <= be).and.((dabs(y_pin(ip) - (y0(i_s) + desy)) <= be))) then
                        call skyr_pinning((x0(i_s) + desx), (y0(i_s) + desy), vpx, vpy, tab_exp, x_pin(ip) - lx, y_pin(ip), u, rp)
                    endif
                enddo
            endif
            ...
            if (esq.and.cima) then
                ...
                do i_ = 1, ncaixa(entx, enty)
                    ip = caixa(entx, enty, i_)
                    if ((dabs(x_pin(ip) - lx - (x0(i_s) + desx)) <= be).and.((dabs(y_pin(ip) + ly - (y0(i_s) + desy)) <= be))) then
                        call skyr_pinning((x0(i_s) + desx), (y0(i_s) + desy), &
                        &vpx, vpy, tab_exp, x_pin(ip) - lx, y_pin(ip) + ly, u, rp)
                    endif
                enddo
            endif
    ...
    fcx = fc * dcos(ac)
    fcy = fc * dsin(ac)
    p1 = ssx + vpx + x_temp + fcx
    p2 = ssy + vpy + y_temp + fcy
